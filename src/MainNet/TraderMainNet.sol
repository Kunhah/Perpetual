...
contract TraderMainNet is Ownable, MainNetPureFunctions {
    using SafeERC20 for IERC20;

    error TraderMainNet_SenderIsNotPerpetual(address sender);

    error TraderMainNet_InvalidPrice(uint80 roundId, int256 priceUSD, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound);

    error TraderMainNet_USDCDepeged();

    error TraderMainNet_wBTCDepeged();
    ...
    //mapping(address => TimeLockedVault) s_tokenToVault;

    address public constant routerAddress = 0xE592427A0AEce92De3Edee1F18E0157C05861564;
    ISwapRouter public immutable i_swapRouter = ISwapRouter(routerAddress);

    //mapping(address => mapping(address => AggregatorV3Interface)) s_tokenToTokenToAggregator;
    ...
    //mapping(address => uint64) immutable priceFeedHeartbeat;
   ...
   error TraderPolygon_SenderIsNotPerpetual(address sender);
    ...
    constructor (address _perpetual, address _owner) Ownable(_owner) {
        ...
        i_vaultCRV = new TimeLockedVault(ADDRESS_CRV);
        ...
        i_vault1INCH = new TimeLockedVault(ADDRESS_1INCH);
        ...
        /* s_tokenToVault[ADDRESS_wBTC] = i_vaultwBTC;
        s_tokenToVault[ADDRESS_wETH] = i_vaultwETH;
        s_tokenToVault[ADDRESS_USDC] = i_vaultUSDC;
        s_tokenToVault[ADDRESS_SHIB] = i_vaultSHIB;
        s_tokenToVault[ADDRESS_PAXG] = i_vaultPAXG;
        s_tokenToVault[ADDRESS_1INCH] = i_vault1INCH;
        s_tokenToVault[ADDRESS_APE] = i_vaultAPE;
        s_tokenToVault[ADDRESS_AAVE] = i_vaultAAVE;
        s_tokenToVault[ADDRESS_AVAX] = i_vaultAVAX;
        s_tokenToVault[ADDRESS_CRV] = i_vaultCRV;
        s_tokenToVault[ADDRESS_COMP] = i_vaultCOMP;
        s_tokenToVault[ADDRESS_MATIC] = i_vaultMATIC;
        s_tokenToVault[ADDRESS_UNI] = i_vaultUNI;
        s_tokenToVault[ADDRESS_SUSHI] = i_vaultSUSHI; */
    
        /* s_tokenToTokenToAggregator[ADDRESS_wBTC][ADDRESS_USDC] = MainNetBTCtoUSD;
        s_tokenToTokenToAggregator[ADDRESS_wETH][ADDRESS_USDC] = MainNetETHtoUSD;
        s_tokenToTokenToAggregator[ADDRESS_1INCH][ADDRESS_USDC] = MainNet1INCHtoUSD;
        s_tokenToTokenToAggregator[ADDRESS_APE][ADDRESS_USDC] = MainNetAPEtoUSD;
        s_tokenToTokenToAggregator[ADDRESS_AVAX][ADDRESS_USDC] = MainNetAVAXtoUSD;
        s_tokenToTokenToAggregator[ADDRESS_AAVE][ADDRESS_USDC] = MainNetAAVEtoUSD;
        s_tokenToTokenToAggregator[ADDRESS_CRV][ADDRESS_USDC] = MainNetCRVtoUSD;
        s_tokenToTokenToAggregator[ADDRESS_COMP][ADDRESS_USDC] = MainNetCOMPtoUSD;
        s_tokenToTokenToAggregator[ADDRESS_MATIC][ADDRESS_USDC] = MainNetMATICtoUSD;
        s_tokenToTokenToAggregator[ADDRESS_UNI][ADDRESS_USDC] = MainNetUNItoUSD;
        s_tokenToTokenToAggregator[ADDRESS_SUSHI][ADDRESS_USDC] = MainNetSUHItoUSD; */
    ...
    function requestTrade(address _tokenIn, uint256 totalCollateralTokenIn, address _tokenOut, uint256 amountBorrowedTokenOut, uint256 assetPrice) external returns (uint256 amountOut) {
        ...
        if(msg.sender != address(perpetual)) revert TraderMainNet_SenderIsNotPerpetual(msg.sender);
    ...
    function haltIfDepeged(address _tokenIn, address _tokenOut) public view {
        ...
        uint8 decimals = wbtcPriceFeed.decimals();
        uint32 heartbeat = getHeartbeat(wbtcPriceFeed);
        bool invalidPrice = roundId == 0 || priceUSDC <= minPrice || priceUSDC >= maxPrice || block.timestamp - updatedAt >= heartbeat || block.timestamp - startedAt <= GRACE_PERIOD;
        if(invalidPrice) revert TraderPolygon_InvalidPrice(roundId, priceUSDC, startedAt, updatedAt, answeredInRound);
   ...
   /*  function ExchangeETHtoERC20(address tokenOut) public payable returns (uint256 amountOut) {
    ...
    function getVault(address token) public returns(TimeLockedVault) {
